...
- `<table class="responsiv rwd-truncate">` — an alternative to scrolling for a wide table: every cell's content is cut to one line with an ellipsis instead of overflowing sideways, with the full value available as a native tooltip (hover or keyboard focus) via `title`. No horizontal scrollbar at all, at the cost of needing to hover to read a truncated value. Still stacks into cards normally below the breakpoint (unlike `rwd-scroll-only`) — only affects the non-stacked, wide-screen view. An existing `title` attribute on a cell is never overwritten.
